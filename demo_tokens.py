...
#        self.qhidden = self.init_hidden2()
    ...
    def forward(self,paragraph,query,s_mask,q_mask):
        ...
        para_encoding = self.paragraph_encoding(para_embeddings,token_feature)
...
#        end_predict_score = end_predict_score * mask
    ...
    def token_features(self,paragraph_batch,query_batch):
        ...
        return autogard.Variable(token_feature_batch_tensor,requires_grad=False)
...
#        return F.dropout(inp,self.dropout,training)
    ...
    def paragraph_encoding(self,embeddings,token_feature):
        embeddings = self.dropout(embeddings)
        paragraph_feature = torch.cat((embeddings,token_feature),2)
        #print(embeddings)
#        self.phidden = self.init_hidden1(len(paragraph))
        lstm_out,self.phidden = self.plstm(paragraph_feature.view(len(embeddings[0]),len(embeddings),EMBEDDING_DIM+2))
