...
class ASReader(nn.Module):
    def __init__(self,embedding_dim,hidden_dim,vocab_size,init_embeddings,word_to_ix,ix_to_word,layer_num,dropout=0.5):
...
#        self.qhidden = self.init_hidden2()
    ...
    def init_hidden1(self,batch_size):
        return (autogard.Variable(torch.randn(2*self.layer_num, batch_size, self.hidden_dim//2), requires_grad=True),
                autogard.Variable(torch.randn(2*self.layer_num, batch_size, self.hidden_dim//2), requires_grad=True))
    ...
    def forward(self,paragraph,query,s_mask,q_mask):
        ...
        token_feature = self.token_features(paragraph.cpu().data.numpy(),query.cpu().data.numpy())
        aligned_embeddings = self.aligned_question_embedding(para_embeddings,query_embeddings)
        para_encoding = self.paragraph_encoding(para_embeddings,token_feature,aligned_embeddings)
...
#        end_predict_score = end_predict_score * mask
    ...
    def aligned_question_embedding(self,para_embeddings,query_embeddings):
        attention = torch.bmm(para_embeddings,query_embeddings.transpose(1,2))
        attention = torch.stack([F.softmax(attention[i]) for i in range(len(attention))],0)
        aligned_embedding = torch.bmm(attention,query_embeddings)
        return aligned_embedding
    
#    def aligned_question_embedding(self,paragraph_batch,query_batch):
#        para_embeddings = self.word_embeddings(paragraph_batch)
#        query_embeddings = self.word_embeddings(query_batch)
#        aligned_embedding = torch.randn(len(para_embeddings),len(para_embeddings[0]),EMBEDDING_DIM)
#        if torch.cuda.is_available():
#            aligned_embedding = aligned_embedding.cuda()
#        aligned_embedding = autogard.Variable(aligned_embedding)
#        weight_value = []
#        for i in range(len(para_embeddings)):
#            values = []
#            for j in range(len(para_embeddings[i])):
#                Z = 0
#                for k in range(len(query_embeddings[i])):
#                    Z += torch.dot(para_embeddings[i][j],query_embeddings[i][k])
#                values.append(Z)
#            weight_value.append(values)
#        for i in range(len(para_embeddings)):
#            sen_level = torch.randn(len(para_embeddings[i]),EMBEDDING_DIM)
#            if torch.cuda.is_available():
#                sen_level = sen_level.cuda()
#            sen_level = autogard.Variable(sen_level)
#            for j in range(len(para_embeddings[i])):
#                vector = torch.zeros(1,EMBEDDING_DIM)
#                if torch.cuda.is_available():
#                    vector = vector.cuda()
#                vector = autogard.Variable(vector)
#                for k in range(len(query_embeddings[i])):
#                    beta = torch.dot(para_embeddings[i][j],query_embeddings[i][k])
#                    vector += query_embeddings[i][k] * beta[0].repeat(EMBEDDING_DIM)
#                sen_level[j] = vector
#            aligned_embedding[i] = sen_level
#        return aligned_embedding
                    
                
        
    def token_features(self,paragraph_batch,query_batch):
        token_feature_batch = []
        for i in range(len(paragraph_batch)):
            token_feature = []
            for w_p in paragraph_batch[i]:
                exact_match = 0
                lower_match = 0
                if w_p == 0:
                    token_feature.append([0,0])
                    continue
                for w_q in query_batch[i]:
                    if(exact_match == 1 and lower_match == 1): break
                    if(ix_to_word[w_p] == ix_to_word[w_q]):
                        exact_match = 1
                    if(ix_to_word[w_p].lower() == ix_to_word[w_q].lower()):
                        lower_match = 1
                token_feature.append([exact_match,lower_match])
            token_feature_batch.append(token_feature)
        token_feature_batch_tensor = torch.Tensor(token_feature_batch)
        if torch.cuda.is_available():
            token_feature_batch_tensor = token_feature_batch_tensor.cuda()            
        return autogard.Variable(token_feature_batch_tensor)
...
#        return F.dropout(inp,self.dropout,training)
    ...
    def paragraph_encoding(self,embeddings,token_feature,aligned_embeddings):
        embeddings = self.dropout(embeddings)
        paragraph_feature = torch.cat((embeddings,aligned_embeddings,token_feature),2)
        #print(embeddings)
#        self.phidden = self.init_hidden1(len(paragraph))
        lstm_out,self.phidden = self.plstm(paragraph_feature.view(len(embeddings[0]),len(embeddings),EMBEDDING_DIM*2+2))
...
#                z += lstm_out[i][j].view(1,-1).mm(self.Wq)
#            weight_list.append(z)
#        
#        #print(lstm_out)
#        result_out = torch.randn(len(lstm_out),HIDDEN_DIM)
#        if torch.cuda.is_available():
#            result_out = result_out.cuda()
#        result_out = autogard.Variable(result_out)
#        for i in range(len(lstm_out)):
#            vector = torch.zeros(1,HIDDEN_DIM)
#            if torch.cuda.is_available():
#                vector = vector.cuda()
#            vector = autogard.Variable(vector)
#            for j in range(len(lstm_out[i])):
#                if mask[i][j].cpu().data.numpy() == 0: continue
#                beta = (lstm_out[i][j].view(1,-1).mm(self.Wq))/(weight_list[i])
#                vector += lstm_out[i][j] * beta[0][0].repeat(self.hidden_dim)
#                #print(beta)
#            result_out[i] = vector.view(-1)
#        #print(result_out)
#        return result_out
...
optimizer = optim.Adamax(parameters,lr=0.002)
